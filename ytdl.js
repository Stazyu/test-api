...
const yt1s = (url, quality = '480p') => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            const res = Object.values(mp4).find(v => v.q === quality);
            ...
            const result = {
                ...
                media: {
                    mp4: {
                        ...
                        //     quality: sdLow.q,
                        //     size: sdLow.size,
                        //     link: await convertLink(sdLow.k, yt.data.vid)
                        // },
                        // '480p': {
                        //     extension: sd.f,
                        //     quality: sd.q,
                        //     size: sd.size,
                        //     link: await convertLink(sd.k, yt.data.vid)
                        // },
                        extension: res.f,
                        quality: res.q,
                        size: res.size,
                        link: await convertLink(res.k, yt.data.vid)
                        // '1080p': {
                        //     extension: fullHd.f,
                        //     quality: fullHd.q,
                        //     size: fullHd.size,
                        //     link: await convertLink(fullHd.k, yt.data.vid)
                        // }
                    },
                    mp3: {
                        extension: audio.f,
